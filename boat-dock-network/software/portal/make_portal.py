...
<div class="card" id="joinCard" style="opacity:.55;pointer-events:none">
  ...
  <div class="opts">
    <label class="opt"><input type="checkbox" id="o_presale" checked><span><b>Reserve my spot</b>a refundable pre-sale deposit that helps fund my zone</span></label>
    <label class="opt"><input type="checkbox" id="o_member" checked><span><b>Become a member</b>join the co-op &amp; start earning WAKE voting units</span></label>
    <label class="opt"><input type="checkbox" id="o_founding"><span><b>Be a founding member</b>invest founding capital (contact me with details)</span></label>
...
document.getElementById('submit').addEventListener('click', async ()=>{{
  ...
  try{{
    ...
    await DB.collection('signups').add(rec);
    document.getElementById('ok').className='ok show';
    document.getElementById('ok').innerHTML='<b>You\\'re on the list for '+rec.zone_name+'.</b> We\\'ll confirm serviceability at survey and reach out about '+
      (rec.founding?'founding membership and ':'')+'your reservation. Welcome to the co-op. 🚤';
    btn.textContent='Reserved ✓'; note.textContent='';
  }}catch(err){{
